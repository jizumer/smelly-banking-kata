...
public class Account implements AccountService {
    ...
    public double calculateAverageBalanceWithBonuses() {
        ...
        for (int i = 0; i < transactions.size(); i++) {
            averageBalanceWithBonuses += transactions.get(i).second() * (1 + Bonuses.calculateBonus(
                getTransactionYear(i)) / 100);
        }
        return averageBalanceWithBonuses / transactions.size();
    }

    private int getTransactionYear(int transactionNumber) {
        return transactions.get(transactionNumber).first().getYear();
    }
}
